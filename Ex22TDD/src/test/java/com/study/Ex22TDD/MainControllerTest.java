...
// @WebMvcTest : @Controller, @RestController가 설정된 컨트롤러 클래스를 테스트한다.
...
class MainControllerTest {
    ...
    void main() throws Exception {
        ...
        mockMvc.perform(get("/"))
                .andExpect(status().isOk())     // HTTP응답코드 : 200
                .andExpect(view().name("index"))
                .andExpect(model().attributeExists("member"))
//                .andExpect(model().attribute("member", member))       // Test에서 생성한 객체와 main에서 생성한 객체는 서로 다름.
